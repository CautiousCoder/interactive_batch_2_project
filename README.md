...
Date: 12-06-2024\
Description: This project is a simple CLI project where add income and expense, show income, expense, total saving and income or expense base category.

Solve this Problem using OOP concept.

## Here project solution :-
```bash
3. class Main{}
```

Declare a PHP class. This Class contain all variable and method.
...
```
get_income_or_expense function return income details or expense details based on the category type.
...
```
set_income_or_expense function store income or expense data in an array.
...
```
get_saving_income function return total saving value.
...
single_string function return unique string for use key of an array.
...
get_category function return all type of incomes or expenses category.

```bash
70. $income_obj = new Main();
```

Create Class instance that use to access the class method/function.

```bash
71. $close_program = false;
72. while (!$close_program) {
```

while loop continue until user exit the program.
...
```
This loop continue if the user choose invalid option.
...
Here different action perform based on the user input.
